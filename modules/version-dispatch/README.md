...
"표로 옮긴다"는 말은 쉬운데 어떤 표인지는 정해야 한다. 세 번 고쳤다.

**첫 번째. 기능과 플랫폼의 2차원.**

```typescript
biometricLogin: { ios: '3.2.0', android: '3.5.0' }
```

가장 단순하다. 그리고 선행 채널이 들어갈 자리가 없다.
문제의 절반을 표현하지 못한다.
...
```typescript
biometricLogin: [
  { platform: 'ios', min: '3.2.0' },
  { platform: 'android', min: '3.5.0' },
  { platform: 'android', channel: 'internal', min: '3.4.0' },
]
```

무엇이든 표현할 수 있다. 조건을 더하고 싶으면 필드를 더하면 된다.

그런데 이 모양은 표가 아니다. 규칙이 몇 개인지, 어떤 순서로 평가되는지가
항목마다 다르다. 위에서 아래로 읽어야 결과를 알 수 있다.
**개선 전 코드의 `if` 사다리를 배열로 옮겨 적은 것에 가깝다.**

애초에 풀려던 문제가 "정책이 한눈에 안 들어온다"였다.
유연한 자료구조는 그 문제를 그대로 둔다.

**세 번째. 정식 개시와 선행 개시, 두 개념만.**
...
일반화를 멈춘 것이 이 모양의 핵심이다.
지금 정책에 있는 개념은 두 개뿐이다. 언제 정식으로 열리는가,
선행 채널은 언제 먼저 받는가. 그 둘만 자리를 만들었다.

덕분에 모든 기능이 같은 모양을 갖는다. 같은 모양이면 세로로 비교할 수 있고,
비교할 수 있으면 빠진 칸이 보인다. 실제로 불일치 3건은 이 모양으로
옮긴 뒤에 눈에 띄었다.

세 번째 개념이 필요해지면 그때 자리를 하나 더 만든다.
...
### 4.0.1처럼 patch까지 적어야 할 때

핫픽스를 4.0.1에 내보내고 "4.0.1부터"라고 적어야 하는 날이 온다.
표에는 이렇게 적는다.

```typescript
inAppPurchase: {
  since: { ios: '4.0.1', android: '4.0.1' },
}
```

**끝이다. 스키마를 바꾸지 않는다.**

`since`는 버전 문자열을 담을 뿐이고 비교는 `Version`이 한다.
`Version.parse`가 세 자리를 모두 읽고 `gte`가 세 자리를 모두 비교한다.
표가 아는 것은 "최소 버전"이라는 개념뿐이고,
그 버전이 몇 자리인지는 표의 관심사가 아니다.
...
**개선 전 코드로는 이걸 적을 수가 없다.**

```typescript
const [major, minor] = appVersion.split('.').map(Number)
```

`patch`를 꺼내지 않는다. 4.0.0과 4.0.9가 같은 값이다.
patch 기준을 넣으려면 이 줄부터 고치고, 그 아래 비교문을 전부 고쳐야 한다.
"3.2 이상"이 두 줄이었듯 "4.0.1 이상"은 세 줄이 된다.

```typescript
if (major > 4) return true
if (major === 4 && minor > 0) return true
if (major === 4 && minor === 0 && patch >= 1) return true
```

이 세 줄짜리 묶음을 네 군데에 복사해야 한다.
기준이 4.0.1인 곳만 세 줄이고 나머지는 두 줄이라, 기능마다 줄 수가 다르다.
**정책을 바꿀 때마다 코드의 모양을 다시 판단하게 된다.**

값 객체로 빼면 그 판단이 없다. `version.gte(Version.parse(min))` 한 줄이
2.9든 3.2든 4.0.1이든 같은 코드다.
...
patch는 되는데 안 되는 것도 있다. 지금 표는 **하한 하나**만 표현한다.
...
| 3.2 이상, iOS만 | 된다 |
| 선행 채널은 3.4부터 | 된다 |
| 4.0.1 이상이되 4.2.0은 제외 | **안 된다** |
| 4.0.0에서 4.0.5까지만 버그가 있어 막아야 함 | **안 된다** |
| 지역이 일본이면 5.0부터 | **안 된다** |

구간과 제외가 들어오면 `since` 문자열 하나로는 부족하다.
`{ min: '4.0.1', exclude: ['4.2.0'] }` 같은 모양이 필요해진다.

그때가 이 구조를 다시 볼 시점이다.
지금 미리 만들어두지 않은 이유는 쓰이지 않는 자리가
읽는 사람에게 부담만 주기 때문이다.
필드가 세 개인 표와 한 개인 표는 훑는 속도가 다르다.

버전 구간 제외가 실제로 필요해진 사례는 대개 하나다.
특정 버전에만 있는 버그다. 그건 정책이 아니라 사고 대응이라,
표에 영구히 남기는 것이 맞는지부터 따져야 한다.
...
읽는 순서가 정책을 그대로 따라간다.
선행 채널이면 그 기준으로 먼저 보고, 안 되면 정식 기준으로 본다.
둘은 OR다. 선행 채널이라도 정식 기준을 넘으면 당연히 열린다.

`min`이 없으면 거짓이다. **표에 칸이 비어 있으면 미지원이라는 규칙이
이 한 줄이다.** 개선 전에는 이게 "해당 `if`가 없음"으로 표현돼 있었다.
없는 것은 눈에 띄지 않는다. 빈 칸은 눈에 띈다.

분기가 두 개다. 기능이 다섯 개든 쉰 개든 이 코드는 늘지 않는다.
늘어나는 것은 표의 행뿐이다.
...
한 번에 바꾸지 않았다. 기존 코드를 지우는 것은 맨 마지막이다.

```mermaid
flowchart TD
    S1["1. 새 구현을 기존 옆에 둔다<br/>아무도 호출하지 않는다"]
    S2["2. 표를 의도대로 채운다<br/>기존 코드를 베끼지 않는다"]
    S3["3. 전수 비교를 돌린다"]
    S4{"차이가 있는가"}
    S5["4. 차이마다 어느 쪽이 맞는지 판정한다"]
    S6["5. 호출부를 새 구현으로 바꾼다"]
    S7["6. 기존 구현은 비교용으로 남긴다"]
...
**2번이 가장 중요하다.** 표를 채울 때 기존 코드를 보고 옮겨 적으면 안 된다.
그러면 버그도 같이 옮겨온다. 옮겨 적은 표는 기존 코드와 100퍼센트 일치하고,
전수 비교는 차이 0을 보고한다. 리팩터링은 성공한 것처럼 보이고
`internal`만 열린 조합은 그대로 남는다.

그래서 표는 "정책이 무엇이어야 하는가"를 보고 채웠다.
선행 채널에 먼저 열어준다는 정책이면 `beta`와 `internal`이 둘 다 들어간다.
그렇게 채우고 나서 기존 코드와 비교하니 3건이 달랐다.
**차이가 나온 것이 실패가 아니라 이 절차가 작동했다는 신호다.**

**5번의 판정은 자동으로 되지 않는다.** 차이 하나하나에 대해
표가 맞는지 기존 코드가 맞는지 사람이 정해야 한다.

이 실험에서는 셋 다 표가 맞다고 봤다. 근거는 세 건이 같은 종류의
실수였다는 점이다. 무작위로 흩어진 차이라면 각각을 의심했을 것이다.

실무라면 여기서 멈추고 확인해야 한다. 그 조합이 의도된 예외일 수 있다.
`beta`에 안 열어준 데에 이유가 있었을지도 모른다.
확인할 사람을 못 찾으면 기존 동작을 유지하고 표에 주석으로 남긴다.
모르는 채로 동작을 바꾸는 것이 가장 나쁘다.

**6번에서 기존 구현을 남긴 이유가 있다.** 전수 비교가 계속 돌아야
새 기능을 추가할 때도 같은 검증을 할 수 있다. 이 저장소에서는
`naive.ts`가 그 자리에 있고 테스트가 두 구현을 비교한다.

실무에서는 한동안 남겼다가 지운다. 남겨두면 두 곳을 고쳐야 하는
부담이 생기므로 영원히 두지는 않는다. 비교가 안정되면 지운다.
...
`src/capabilities.ts`의 `inAppPurchase`에서 `since`를 바꾼다.
...
불일치가 3건에서 5건으로 늘어난다. 개선 전 코드는 patch를 읽지 않아
4.0.0을 열어두기 때문이다. 표만 고쳐서 표현할 수 있는 기준이 개선 전 코드에는
아예 없다는 것을 숫자로 볼 수 있다.

되돌린 뒤 값 객체만 따로 확인할 수도 있다.

```bash
npx vitest run modules/version-dispatch -t "4.0.1"
```
