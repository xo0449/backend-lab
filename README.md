...
| [forwardRef를 붙였더니 에러는 사라졌는데 생성자에서 본 값이 undefined였다](modules/nest-circular) | 실험 | NestJS 모듈이 서로를 import할 때 순환이 생기면 무엇이 언제 깨지는가 |
...
npm run lab:circular      # NestJS 순환 참조와 forwardRef
```
